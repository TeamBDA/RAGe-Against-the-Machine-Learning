...
MODEL_NAME     = "vosk-model-en-us-0.22-lgraph"  # ← changed from vosk-model-small-en-us-0.15
OUTPUT_CSV     = os.path.join(BASE_DIR, "data", "transcriptions_v22.csv")     # ← changed to avoid overwriting v15 output
METRICS_CSV     = os.path.join(BASE_DIR, "data", "metrics_V22.csv")  #captures the time taken for each transcription and accuracy metrics
...
def compute_wer(reference, hypothesis):
    ...
    A WER of 0.0 means a perfect match; 1.0 means every word was wrong.
    ...
    for i in range(len(ref) + 1): #This answers: "how many edits to match the first i reference words against an empty hypothesis?"  
        d[i][0] = i #first column lays out the VOSK model word output
    for j in range(len(hyp) + 1): #"how many edits to match an empty reference against the first j hypothesis words?" 
        d[0][j] = j # first row now shows each word of the correct transcript
